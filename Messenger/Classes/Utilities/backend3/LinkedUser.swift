...
class LinkedUser: NSObject {
	...
	class func user(userId: String) -> [String: Any] {
		...
		user[FUSER_OBJECTID]		= dbuser.objectId

		user[FUSER_EMAIL]			= dbuser.email
		user[FUSER_PHONE]			= dbuser.phone

		user[FUSER_FIRSTNAME]		= dbuser.firstname
		user[FUSER_LASTNAME]		= dbuser.lastname
		user[FUSER_FULLNAME]		= dbuser.fullname
		user[FUSER_COUNTRY]			= dbuser.country
		user[FUSER_LOCATION]		= dbuser.location
		user[FUSER_STATUS]			= dbuser.status

		user[FUSER_PICTURE]			= dbuser.picture
		user[FUSER_THUMBNAIL]		= dbuser.thumbnail
		...
		user[FUSER_WALLPAPER]		= dbuser.wallpaper

		user[FUSER_LOGINMETHOD]		= dbuser.loginMethod
		user[FUSER_ONESIGNALID]		= dbuser.oneSignalId
